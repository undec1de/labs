...
void Add(Tree*& T, Book data, bool flag) {
    if (!T) {
        T = new Tree;
        T->data = data;
        T->left = NULL;
    ...
    if (flag) {
        ...
        if (data.publication < T->data.publication) Add(T->left, data, 0);
...
Tree* SearchStr(Tree*& T, string data) {
    if (!T) return NULL;
    if (data[0] == T->data.publication[0]) {
        T->data.Out();
    }
...
void readFile(Tree*& T) {
    Book w;
    system("cls");
    fstream f_in("book.txt", ios::in);
    if (!f_in) {
        cout << "Файл не найден" << endl;
        return;
    }
    while (f_in >> w.publication >> w.color >> w.price >> w.pages) {
        Add(T, w, 1);
    }
    f_in.close();
}
...
void Menu() {
    ...
    do {
        system("cls");
        cout << "1. Сортировка по издательству" << endl;
        cout << "2. Сортировка по цене" << endl;
        cout << "3. Выбрать элемент по издательству" << endl;
